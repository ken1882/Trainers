...
def ocr_rect(rect, fname, zoom=1.0, lang='jpn', config='--psm 12 --psm 13', color=None, color_bias=None, **kwargs):
  ...
  if color is not None:
    if color_bias is None:
      color_bias = _G.ColorBiasRange
    color = np.array(color[:3], dtype=np.int16)
    arr = np.array(img.convert('RGB'), dtype=np.int16)
    mask = np.all(np.abs(arr - color) <= color_bias, axis=2)
    arr[:] = 0
    arr[mask] = 255
    img.close()
