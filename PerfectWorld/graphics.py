...
def print_window(hwnd=None):
  ...
  # Create a device context in memory and a compatible bitmap
  hdc_mem = win32ui.CreateDCFromHandle(win32gui.CreateCompatibleDC(hdc_window))
  bitmap = win32ui.CreateBitmap()
  bitmap.CreateCompatibleBitmap(win32ui.CreateDCFromHandle(hdc_window), width, height)
  # Select the compatible bitmap into the memory DC
  hdc_mem.SelectObject(bitmap)
  # BitBlt: Copy the client area into the memory device context
  windll.user32.PrintWindow(hwnd, hdc_mem.GetSafeHdc(), 3)
  # Convert the bitmap to a format usable in Python (optional)
  ...
  hdc_mem.DeleteDC()
  # You can now save or process bmpstr (e.g., using PIL)
  img = Image.frombuffer(
    'RGB',
    (bmpinfo['bmWidth'], bmpinfo['bmHeight']),
    bmpstr, 'raw', 'BGRX', 0, 1
  )
  path = f"{_G.DCTmpFolder}/{_G.DCSnapshotFile}"
  img.save(path)
  img = Image.open(path)
  return img 
...
def take_snapshot(rect=None,filename=None):
  ...
  if not FlagDisableCache and _G.LastFrameCount == _G.FrameCount and filename in _G.SnapshotCache:
