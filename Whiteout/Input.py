...
def key_down(*args, use_msg=_G.AppInputUseMsg, hwnd=None):
  ...
  for kid in args:
    if use_msg:
      lp = 0
      win32api.SendMessage(hwnd, win32con.WM_KEYDOWN, kid, lp)
    else:
...
def key_up(*args, use_msg=_G.AppInputUseMsg, hwnd=None):
  ...
  for kid in args:
    if use_msg:
      lp = 0
      win32api.SendMessage(hwnd, win32con.WM_KEYUP, kid, lp)
    else:
...
def trigger_key(*args, use_msg=_G.AppInputUseMsg, hwnd=None):
  if not hwnd:
    hwnd = _G.AppInputHwnd
  key_down(*args, use_msg=use_msg)
  sleep(0.03)
  key_up(*args, use_msg=use_msg)
