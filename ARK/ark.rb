...
if SelectedWorker
  ...
  puts "No worker selected, default set to intertact"
  SelectedWorker = :start_interact_fiber
end
...
def main_update
  ...
  if Input.trigger?(Keymap[:vk_f8]) || ($flag_auto_restart && !$flag_first_work)
    ...
    if $flag_working
      $fiber = Fiber.new{method(SelectedWorker).call(*ARGV[1..-1])} if SelectedWorker
    else
